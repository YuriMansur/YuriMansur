...
class MainWindow(QWidget):
    def __init__(self, plc_manager):
        ...
        self.labels = {}  # key = "PLC_poll", value = QLabel
        self.add_label(layout, "PLC1", "holding_0_10")
        self.add_label(layout, "PLC1", "holding_100_5")
        ...
        # QTimer для циклического обновления GUI
        self.timer = QTimer()
    ...
    def update_gui(self):
        for key, label in self.labels.items():
            ...
            label.setText(f"{key}: {value}" if value else f"{key}: нет данных")
