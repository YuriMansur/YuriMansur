...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QLineEdit, QLabel, QCheckBox, QProgressBar, QTreeWidget, QTreeWidgetItem
)
...
class App(QWidget):
    ...
    def load_tree(self):
        """Загрузить и построить полное рекурсивное дерево"""
        self.tree.clear()
        try:
            ...
            self.tree.expandAll()
            self.log.append("🌳 Full tree loaded successfully")
        except Exception as e:
            self.log.append(f"❌ Failed to load tree: {e}")

    def build_tree(self, parent_item, data):
        """Рекурсивное построение дерева из словаря"""
        for k, v in data.items():
            if not isinstance(v, dict):
                continue
            node_id = v.get("node_id", "")
            item = QTreeWidgetItem([k, node_id])
            parent_item.addChild(item)
            # Рекурсивно добавляем дочерние узлы
            children = {kk: vv for kk, vv in v.items() if isinstance(vv, dict) and ("node_id" in vv or vv)}
            if children:
